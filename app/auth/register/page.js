...
function RegisterForm() {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      console.log('✅ User berhasil registrasi:', data.user.email)
    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-200/60 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-800 mb-2">Daftar Akun Baru</h1>
          <p className="text-sm text-slate-500">
            Sudah punya akun?{' '}
            <Link href={`/auth/login?redirect=${encodeURIComponent(redirectTo)}`} className="text-blue-600 hover:underline font-medium">
              Masuk di sini
            </Link>
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div className="space-y-1">
            ...
            <input
              ...
              disabled={success}
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition disabled:opacity-50"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">No. HP / WhatsApp</label>
            <input
              required
              type="tel"
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Konfirmasi Password</label>
            <input
              required
              type="password"
